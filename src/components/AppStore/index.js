...
class AppStore extends Component {
  ...
  MainChangePortal = tabValue => {
    this.setState({tabIds: tabValue})
  }

  GettingParticular = event => {
    this.setState({searchData: event.target.value})
  }

  filterContent = search => {
    const {tabIds} = this.state
    const values = search.filter(each => each.category === tabIds)
  ...
  SearchingData = () => {
    ...
    const SearchingData = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchData.toLowerCase()),
    )
    return SearchingData
  }

  render() {
    const {tabIds, searchData} = this.state
    const search = this.SearchingData()
    const filteredData = this.filterContent(search)
        ...
        <ul className="rowing">
          {tabsList.map(each => (
            <TabItem
              each={each}
              key={each.TabId}
              MainChangePortal={this.MainChangePortal}
              isActive={each.TabId === tabIds}
            />
          ))}
        </ul>
        <hr />
        <ul className="rowingimages">
